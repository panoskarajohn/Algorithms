...
public class MinimumDifficultyJobSchedule
{
    ...
    public int MinDifficultyBottomUp(int[] jobDifficulty, int days)
    {
        ...
        {
            dp[i] = new int[n + 1];
            for (var j = 0; j < n; j++) dp[i][j] = int.MaxValue;
        }

        for (var daysRemaining = 1; daysRemaining <= days; daysRemaining++)
        for (var i = 0; i < n - daysRemaining + 1; i++)
        {
            var dailyMaxDiff = 0;
            for (var j = i + 1; j < n - daysRemaining + 2; j++)
            {
    ...
    {
        var n = jobDifficulty.Length;
        var minDiffNextDay = new int[n + 1];
        for (var i = 0; i < n; i++)
            minDiffNextDay[i] = int.MaxValue;

        for (var daysRemaining = 1; daysRemaining <= days; daysRemaining++)
        {
            var minDiffCurrDay = new int[n + 1];
            for (var i = 0; i < n; i++) minDiffCurrDay[i] = int.MaxValue;
