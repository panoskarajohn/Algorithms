...
public static class MinimumWindowSubstring
{
    public static string Get(string s, string t)
    {
        ...
        int requiredCharacters = dictionaryT.Count;
        int formed = 0; 
        ...
        while (right < s.Length)
        {
            ...
            if (dictionaryT.ContainsKey(current) && dictionaryT[current] == dictionaryWindow[current])
            {
                formed++;
            }
            ...
            while (left <= right && formed == requiredCharacters)
            {
                ...
                if (dictionaryT.ContainsKey(leftChar) && dictionaryWindow[leftChar] < dictionaryT[leftChar])
                {
                    formed--;
                }
