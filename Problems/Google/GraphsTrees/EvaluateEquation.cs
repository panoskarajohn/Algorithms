...
public static class EvaluateEquation
{
        ...
        IList<IList<string>> queries)
    {
        ...
        foreach (var query in queries)
        {
            ...
            var result = DFS(query[0], query[1], graph, visited);
            answers[i++] = result;
        }
        ...
        HashSet<string> visited)
    {
        ...
        foreach (var entry in startMap)
            ...
            {
                var current = DFS(entry.Key, end, graph, visited);
        ...
        , double[] values)
    {
        ...
        for (var i = 0; i < n; i++)
        {
            ...
            if (!graph.ContainsKey(num)) graph.Add(num, new Dictionary<string, double>());

            if (!graph.ContainsKey(den)) graph.Add(den, new Dictionary<string, double>());
